...
// from http://chir.ag/projects/ntc
...
const COLOR_LOOKUP = Object.fromEntries(COLOR_NAMES)

const HEX_LOOKUP = COLOR_NAMES.map(([hex]) => hex)

// [ valid, exactMatch, colorName, colorHex]
export default (color) => {
  // invalid
  if (!/^\#[0-9a-h]{6}$/i.test(color)) return [false]

  // match
  const hex = color.split('').slice(1).join('').toUpperCase()
  const colorName = COLOR_LOOKUP[hex]
  if (colorName) return [true, true, colorName, `#${hex}`]

  // no match
  const closestMatchingHex = binarySearch(HEX_LOOKUP, hex)
  const closestMatchingName = COLOR_LOOKUP[closestMatchingHex]
  return [true, false, closestMatchingName, `#${closestMatchingHex}`]
}
